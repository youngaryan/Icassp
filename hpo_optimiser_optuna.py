...
# CSV file setup
csv_file = os.path.join(os.path.dirname(__file__), "hpo_results_bca1_test.csv")
fieldnames = [
...
def objective(trial):
    ...
    max_length = trial.suggest_categorical("max_length",
                                           [2 * 16000, 3 * 16000, 4 * 16000, 5 * 16000, 7 * 16000, 10 * 16000])
    ...
    return results.get("validation_accuracy", 0), total_time
...
if __name__ == "__main__":
    ...
    print(f"\nFound {len(pareto)} Pareto-optimal trial(s).")
    for t in pareto:
        acc, sec = t.values  # (validation_accuracy, total_time)
        print("\nPareto trial:")
        print(f"  Trial #: {t.number}")
        print(f"  Validation Accuracy (BCA): {acc:.6f}")
        print(f"  Total Time (s): {sec:.2f}")
        print(f"  Config: {t.user_attrs['config']}")
        print(f"  Full Results: {t.user_attrs['results']}")

    print(f"\nHPO results written to {csv_file}")