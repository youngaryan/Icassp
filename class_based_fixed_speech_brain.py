...
from dataset_speech_brain import EmotionDataset
    ...
    def train(self):
        ...
        for epoch in range(self.config["num_epochs"]):
            ...
            for batch_idx, (inputs, targets) in enumerate(self.train_loader):
                ...
                total_samples += targets.size(0)
                print(
                    f"Epoch [{epoch + 1}/{self.config['num_epochs']}], Batch [{batch_idx + 1}/{len(self.train_loader)}], Loss: {loss.item():.4f}")
            ...
            print(
                f"Epoch [{epoch + 1}/{self.config['num_epochs']}], Average Loss: {epoch_loss:.4f}, Training Accuracy: {epoch_accuracy:.4f}")
